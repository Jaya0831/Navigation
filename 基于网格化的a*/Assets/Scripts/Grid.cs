...
public class Grid : MonoBehaviour
{
    ...
    private void TransformTilemapToGrid(Tilemap mytilemap, int left, int right, int up, int down)//最（左/右/上/下）的坐标
    {
        for (int i = left; i <= right; i++)
        {
            for (int j = up; j <= down; j++)
            {
                if (mytilemap.GetSprite(new Vector3Int(i,j,0))==sprite_obstacle)
                {
                    grid[i-left, j-up].status = Node.Status.obstacle;
                }
            }
        }
    }
